...
fit_gam_fun <- function (sci_name, directory, model_terms, naive_terms, fit_gam = TRUE, var_imp = TRUE, gam_stats = TRUE) {
  ...
  for (i in 1:length(sci_name)) {
    ...
    # kg_tot is survey tow biomass in kg per nautical mile
    # model formula in paper: g(mu)=ntercept + s(tow depth) + s(rugosity) + s(SST) + s(BT) + s(min SST) + s(max SST) + s(max BT)
    ...
    if (fit_gam == TRUE) {
      ...
      save (gam_tw,  file = paste0("C:/Users/jmason/iceland_climate_resilient_fisheries/Models/", directory, "/", sci_name[i], ".Rdata"))
    ...
    if (var_imp == TRUE) {
      ...
      for (x in model_terms) {
        ...
        df <- data.frame (species = sci_name[i],
                          ...
                          Diff_f = round (summary (gam_tw)$dev.expl * 100 - summary(G_drop)$dev.expl * 100, 2)
        )
...
spp_done_Files <- list.files ("Models/Rug_tw_LL")
spp_done <- map_chr (spp_done_Files, scan_fun)

spp_remain <- borm_spp$sci_name_underscore[which (!borm_spp$sci_name_underscore %in% spp_done)]
