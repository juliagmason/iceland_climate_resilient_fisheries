...
Fishbase: 
Depth range 10-3000m, usually  90-250
Temp: -0.5 to 2.5 bottom temp; preferred temp 0.6-10.3 mean 5.6 (probably sst)
35cm maturity, max 82, max weight 6.4 

widespread north atlantic
soft bottoms, sinverts small fishes
...
# sample ID of all lr_dab presences and absences
lr_dab_pa <- read_csv ("../Data/PA_MFRI.csv",
                     col_types = cols(
                       sample_id = col_factor()
                     )) %>%
  dplyr::select (sample_id, Hippoglossoides_platessoides)

# full MFRI info for all lr_dab presences, retaining length data
lr_dab_samp_full <- read_csv ("../Data/MFRI_comb_survey.csv",
                      col_types = cols(
                      sample_id = col_factor(),
                      species = col_factor(),
                      stat_sq = col_factor()
  ...
  filter (species == 28, !(year == 2011 & season == "autumn")) # remove autumn 2011
  
# all lr_dab presences, grouped by sample_ID (lengths aggregated
lr_dab_samp <- lr_dab_samp_full %>%
  group_by (sample_id) %>%
...
```
Present in `r nrow (lr_dab_samp)` samples (`r nrow(lr_dab_samp)/29002 * 100`%). very common
...
```
larger in spring. smoothest spread yet

## biomass and abundance dotcharts
```{r dotcharts}
dotchart(lr_dab_samp$n_tot, 
         groups = factor(lr_dab_samp$season), 
         main = "Abundance")

dotchart(lr_dab_samp$n_log, 
         #groups = factor(lr_dab_samp$season), 
         main = "log abundance")

```
2 above 200, 1994 and 2015
...
lr_dab_samp$n_tot[which (lr_dab_samp$n_tot > 10)] > quantile (lr_dab_samp$n_tot, 0.75) + 1.5 * (quantile (lr_dab_samp$n_tot, 0.75) - quantile (lr_dab_samp$n_tot, 0.25)) # yes
...
```
big spike in mid 0-s through early 2000s in spring. both spring and fall seem to show slight recovery in 2012-2016ish.  
...
ggplot(data = world) +
  ...
  stat_summary_2d(data = lr_dab_samp, aes (x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.25, .25)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
    facet_wrap (~season)
```
all around but inshore. more inshore in fall, maybe even mostly north. 


```{r space_by_length, warning = FALSE}
length_bins = quantile (lr_dab_samp_full$length_cm)
length_bins[1] <- 0 # include smallest ones

lr_dab_length_bins <- lr_dab_samp_full %>%
...
ggplot(data = world) +
  geom_sf() +
  coord_sf (xlim = c(-30, -8), ylim = c (62,70), expand = FALSE ) +
  stat_summary_2d(data = lr_dab_length_bins, aes(x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.5, .5)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
...
```{r lr_dab_preds, warning = FALSE}
lr_dab_ab_pred <- lr_dab_samp %>%
  dplyr::select (sample_id, n_log, kg_log) %>%
  right_join (mfri_pred, by = "sample_id")

bt_p <- ggplot (lr_dab_ab_pred, aes (x = bottom_temp, y = kg_log)) +
  geom_point(alpha = 0.5) +
  geom_smooth (method = "lm") +
  ggtitle ("Bottom temp")
  

sst_p <- ggplot (lr_dab_ab_pred, aes (x = surface_temp, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Surface temp")

sal_p <- ggplot (lr_dab_ab_pred, aes (x = gins_sal, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Salinity")

lat_p <- ggplot (lr_dab_ab_pred, aes (x = lat, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Latitude")

lon_p <- ggplot (lr_dab_ab_pred, aes (x = lon, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Longitude")

depth_p <-  ggplot (lr_dab_ab_pred, aes (x = tow_depth_begin, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Depth")
...
```
negative with everything except lat and long!

### Model results
```{r model_explore}
library (mgcv)
load ("../Models/PresAbs/Hippoglossoides_platessoides_PA_full.Rdata")
summary (gam_PA)

```
year, surface temp, bottom temp not significant, sal not great 
...
save (single.var.comp, file = "../Models/PresAbs_summary_tables/lr_dab_single_var_comp.RData")

```
year, season, salinity make worse

```{r biomass_gam}

load ("../Models/Biomass/Hippoglossoides_platessoides_LB_full.Rdata")
summary (gam_LB)
```
...
for (x in full.terms){
  ...
  G <- gam (formula, family = gaussian, data = lr_dab_ab_pred)
  
  # full model without that variable
 formula_drop <- as.formula (paste0 ("kg_log ~", (paste (full.terms[! full.terms %in% x], collapse = " + "))))
 G_drop <- gam (formula_drop, family = gaussian, data = filter (lr_dab_ab_pred, !is.na (x)))
...
save (single.var.comp, file = "../Models/Biomass_summary_tables/lr_dab_single_var_comp.RData")

```
everything conributes. year actually biggest contributor behind depth. 