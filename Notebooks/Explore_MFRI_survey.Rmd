...
I basically want a presence/absence table, but with stations
```{r station_duration}
...
680 stations in spring (still more than there are stations??); 436 in all years, 518 > 30. 414 in autumn, 280 above 20. No overlap?
### Spatial extent
```{r tow_locations}
...
I can only think of how to do this separately for autumn and spring. spread by year and then take sum?
```{r sampled_all_yrs}
...
length(which (spp_yrs_sampled$season =="spring" & spp_yrs_sampled$n_yrs_sampled > 32)) # 27
length(which (spp_yrs_sampled$season =="autumn" & spp_yrs_sampled$n_yrs_sampled > 20)) # 29, 61 > 20
...
```
27 spp sampled every spring, 29 every autumn. 37 have more than 30, 31 have more than 32 in spring. 25 in both
