...
Fishbase: 
Depth range 20-1000m, usually 100-400. 
Temp: 3.8-14.2, mean 7.7
5range 35-60cm, max 200, common 100 max 180, max wt 57kg

eastern atlantic, mostly north sea, down to portugal, southern iceland
sandy muddy bottoms, 
diet: fishes and sea birds??
...
# sample ID of all monk presences and absences
monk_pa <- read_csv ("../Data/PA_MFRI.csv",
                     col_types = cols(
                       sample_id = col_factor()
                     )) %>%
  select (sample_id, Lophius_piscatorius)

# full MFRI info for all monk presences, retaining length data
monk_samp_full <- read_csv ("../Data/MFRI_comb_survey.csv",
                      col_types = cols(
                      sample_id = col_factor(),
                      species = col_factor(),
                      stat_sq = col_factor()
  ...
  filter (species == 14, !(year == 2011 & season == "autumn")) # remove autumn 2011
  
# all monk presences, grouped by sample_ID (lengths aggregated
monk_samp <- monk_samp_full %>%
  group_by (sample_id) %>%
...
```
Present in `r nrow (monk_samp)` samples (`r nrow(monk_samp)/29002 * 100`%). Quite few. 
...
```
Bimodal fall, smaller in spring 

## biomass and abundance dotcharts
```{r dotcharts}
dotchart(monk_samp$n_tot, 
         groups = factor(monk_samp$season), 
         main = "Abundance")

dotchart(monk_samp$n_log, 
         #groups = factor(monk_samp$season), 
         main = "log abundance")
...
```
really different biomass vs abundance. this is one of the "big story species" but I wouldn't get that from looking at biomass. abundance spikes until 2010, then falls, particularly in spring. but biomass is in a dip from 1995-2007ish, then back up to past levels. 
...
ggplot(data = world) +
  ...
  stat_summary_2d(data = monk_samp, aes (x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.25, .25)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
    facet_wrap (~season)
```
South and west, more reykjanes 


```{r space_by_length, warning = FALSE}
length_bins = quantile (monk_samp_full$length_cm)
length_bins[1] <- 0 # include smallest ones

monk_length_bins <- monk_samp_full %>%
...
ggplot(data = world) +
  geom_sf() +
  coord_sf (xlim = c(-30, -8), ylim = c (62,70), expand = FALSE ) +
  stat_summary_2d(data = monk_length_bins, aes(x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.5, .5)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
...
```{r monk_preds, warning = FALSE}
monk_ab_pred <- monk_samp %>%
  dplyr::select (sample_id, n_log, kg_log) %>%
  right_join (mfri_pred, by = "sample_id")

bt_p <- ggplot (monk_ab_pred, aes (x = bottom_temp, y = kg_log)) +
  geom_point(alpha = 0.5) +
  geom_smooth (method = "lm") +
  ggtitle ("Bottom temp")
  

sst_p <- ggplot (monk_ab_pred, aes (x = surface_temp, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Surface temp")

sal_p <- ggplot (monk_ab_pred, aes (x = gins_sal, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Salinity")

lat_p <- ggplot (monk_ab_pred, aes (x = lat, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Latitude")

lon_p <- ggplot (monk_ab_pred, aes (x = lon, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Longitude")

depth_p <-  ggplot (monk_ab_pred, aes (x = tow_depth_begin, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Depth")
...
```
strong positive sith temp, sal, depth. neg latitude 

### Model results
```{r model_explore}
library (mgcv)
load ("../Models/PresAbs/Lophius_piscatorius_PA_full.Rdata")
summary (gam_PA)
...
save (single.var.comp, file = "../Models/PresAbs_summary_tables/monk_single_var_comp.RData")

```

everything contributes. salinity makes the biggest difference from full, then year, then lat/lon, although lat/lon has biggest r2 single. bottom temp and salinity biggest r2 single. 
```{r biomass_gam}

load ("../Models/Biomass/Lophius_piscatorius_LB_full.Rdata")
summary (gam_LB)
```
surface temp and bottom temp not significant, nor season. only 1800 samples
...
for (x in full.terms){
  ...
  G <- gam (formula, family = gaussian, data = monk_ab_pred)
  
  # full model without that variable
 formula_drop <- as.formula (paste0 ("kg_log ~", (paste (full.terms[! full.terms %in% x], collapse = " + "))))
 G_drop <- gam (formula_drop, family = gaussian, data = filter (monk_ab_pred, !is.na (x)))
...
save (single.var.comp, file = "../Models/Biomass_summary_tables/monk_single_var_comp.RData")

```
salinity makes a lot worse. lat/.on, then depth, then year.   