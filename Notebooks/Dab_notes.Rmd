...
# sample ID of all dab presences and absences
dab_pa <- read_csv ("../Data/PA_MFRI.csv",
                     col_types = cols(
                       sample_id = col_factor()
                     )) %>%
  dplyr::select (sample_id, Limanda_limanda)

# full MFRI info for all dab presences, retaining length data
dab_samp_full <- read_csv ("../Data/MFRI_comb_survey.csv",
                      col_types = cols(
                      sample_id = col_factor(),
                      species = col_factor(),
                      stat_sq = col_factor()
  ...
  filter (species == 27, !(year == 2011 & season == "autumn")) # remove autumn 2011
  
# all dab presences, grouped by sample_ID (lengths aggregated
dab_samp <- dab_samp_full %>%
  group_by (sample_id) %>%
...
```
Present in `r nrow (dab_samp)` samples (`r nrow(dab_samp)/29002 * 100`%).
...
```
larger in spring. fairly similar/normal spread

## biomass and abundance dotcharts
```{r dotcharts}
dotchart(dab_samp$n_tot, 
         groups = factor(dab_samp$season), 
         main = "Abundance")

dotchart(dab_samp$n_log, 
         #groups = factor(dab_samp$season), 
         main = "log abundance")
...
dab_samp$n_tot[which (dab_samp$n_tot > 10)] > quantile (dab_samp$n_tot, 0.75) + 1.5 * (quantile (dab_samp$n_tot, 0.75) - quantile (dab_samp$n_tot, 0.25)) # yes
...
```
fairly constant. maybe a spike in the early 2000s. late 90s for autumn.  
...
ggplot(data = world) +
  ...
  stat_summary_2d(data = dab_samp, aes (x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.25, .25)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
    facet_wrap (~season)
```
west and south, inshore


```{r space_by_length, warning = FALSE}
length_bins = quantile (dab_samp_full$length_cm)
length_bins[1] <- 0 # include smallest ones

dab_length_bins <- dab_samp_full %>%
...
ggplot(data = world) +
  geom_sf() +
  coord_sf (xlim = c(-30, -8), ylim = c (62,70), expand = FALSE ) +
  stat_summary_2d(data = dab_length_bins, aes(x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.5, .5)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
...
```{r dab_preds, warning = FALSE}
dab_ab_pred <- dab_samp %>%
  dplyr::select (sample_id, n_log, kg_log) %>%
  right_join (mfri_pred, by = "sample_id")

bt_p <- ggplot (dab_ab_pred, aes (x = bottom_temp, y = kg_log)) +
  geom_point(alpha = 0.5) +
  geom_smooth (method = "lm") +
  ggtitle ("Bottom temp")
  

sst_p <- ggplot (dab_ab_pred, aes (x = surface_temp, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Surface temp")

sal_p <- ggplot (dab_ab_pred, aes (x = gins_sal, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Salinity")

lat_p <- ggplot (dab_ab_pred, aes (x = lat, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Latitude")

lon_p <- ggplot (dab_ab_pred, aes (x = lon, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Longitude")

depth_p <-  ggplot (dab_ab_pred, aes (x = tow_depth_begin, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Depth")
...
```
very strong negative with depth. pos with temp and sal. neg lat, pos lon

### Model results
```{r model_explore}
library (mgcv)
load ("../Models/PresAbs/Limanda_limanda_PA_full.Rdata")
summary (gam_PA)

```
year, surface temp, salinity not significant. 
...
save (single.var.comp, file = "../Models/PresAbs_summary_tables/dab_single_var_comp.RData")

```
salinity makes much worse, year and season also make worse. 

```{r biomass_gam}

load ("../Models/Biomass/Limanda_limanda_LB_full.Rdata")
summary (gam_LB)
```
season not significant, salinity not significant
...
for (x in full.terms){
  ...
  G <- gam (formula, family = gaussian, data = dab_ab_pred)
  
  # full model without that variable
 formula_drop <- as.formula (paste0 ("kg_log ~", (paste (full.terms[! full.terms %in% x], collapse = " + "))))
 G_drop <- gam (formula_drop, family = gaussian, data = filter (dab_ab_pred, !is.na (x)))
...
save (single.var.comp, file = "../Models/Biomass_summary_tables/dab_single_var_comp.RData")

```
salinity makes worse. lat/lon and depth most important.  