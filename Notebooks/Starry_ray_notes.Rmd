...
Fishbase: 
Depth range 5-1540m, usually 25-440 
Temp: -1 - 14, common 2-5
87.5cm, maturity max 105, max wt 11.4kg

US northeast and nortn atlantic, arctic
offshore waters, all kinds of bottoms, sandy and muddy
diet: fish, crustaceans, polychaets, also scavenger, hydroids, molluscs, cephs, echinos
...
# sample ID of all starry presences and absences
starry_pa <- read_csv ("../Data/PA_MFRI.csv",
                     col_types = cols(
                       sample_id = col_factor()
                     )) %>%
  select (sample_id, Amblyraja_radiata)

# full MFRI info for all starry presences, retaining length data
starry_samp_full <- read_csv ("../Data/MFRI_comb_survey.csv",
                      col_types = cols(
                      sample_id = col_factor(),
                      species = col_factor(),
                      stat_sq = col_factor()
  ...
  filter (species == 12, !(year == 2011 & season == "autumn")) # remove autumn 2011
  
# all starry presences, grouped by sample_ID (lengths aggregated
starry_samp <- starry_samp_full %>%
  group_by (sample_id) %>%
...
```
Present in `r nrow (starry_samp)` samples (`r nrow(starry_samp)/29002 * 100`%). Middling
...
```
Many more in spring. similar spread, maybe larger lean in fall

## biomass and abundance dotcharts
```{r dotcharts}
dotchart(starry_samp$n_tot, 
         groups = factor(starry_samp$season), 
         main = "Abundance")

dotchart(starry_samp$n_log, 
         #groups = factor(starry_samp$season), 
         main = "log abundance")

```
One big outlier above 60 (56868, 1993), one above 20 (579486, 2012), two maybe above 15
```{r outlier_check}

starry_samp %>%
  filter (n_tot > 15)

starry_samp$n_tot[which (starry_samp$n_tot > 15)] > quantile (starry_samp$n_tot, 0.75) + 1.5 * (quantile (starry_samp$n_tot, 0.75) - quantile (starry_samp$n_tot, 0.25)) # yes
...
```
Pretty marked declines in both, not really seeing anything concerining from outlier points  
...
ggplot(data = world) +
  ...
  stat_summary_2d(data = starry_samp, aes (x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.25, .25)) +
...
length_bins = quantile (starry_samp_full$length_cm)
length_bins[1] <- 0 # include smallest ones

starry_length_bins <- starry_samp_full %>%
...
ggplot(data = world) +
  geom_sf() +
  coord_sf (xlim = c(-30, -8), ylim = c (62,70), expand = FALSE ) +
  stat_summary_2d(data = starry_length_bins, aes(x = lon, y = lat, z = n_log), fun = mean, binwidth = c (.5, .5)) +
    scale_fill_viridis_c(name = "Mean log abundance") +
    theme_bw() +
...
```{r starry_preds, warning = FALSE}
starry_ab_pred <- starry_samp %>%
  dplyr::select (sample_id, n_log, kg_log) %>%
  right_join (mfri_pred, by = "sample_id")

bt_p <- ggplot (starry_ab_pred, aes (x = bottom_temp, y = kg_log)) +
  geom_point(alpha = 0.5) +
  geom_smooth (method = "lm") +
  ggtitle ("Bottom temp")
  

sst_p <- ggplot (starry_ab_pred, aes (x = surface_temp, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Surface temp")

sal_p <- ggplot (starry_ab_pred, aes (x = gins_sal, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Salinity")

lat_p <- ggplot (starry_ab_pred, aes (x = lat, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Latitude")

lon_p <- ggplot (starry_ab_pred, aes (x = lon, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Longitude")

depth_p <-  ggplot (starry_ab_pred, aes (x = tow_depth_begin, y = kg_log)) +
  geom_point() +
  geom_smooth (method = "lm")+
  ggtitle ("Depth")
...
```
also negative with temp, like wolffish. both surface and bottom. also strong negative with salinity. positive with lat and lon, maybe slightly positive with depth  

### Model results
```{r model_explore}
library (mgcv)
load ("../Models/PresAbs/Amblyraja_radiata_PA_full.Rdata")
summary (gam_PA)
...
save (single.var.comp, file = "../Models/PresAbs_summary_tables/starry_single_var_comp.RData")

```

year makes worse. nothing really standing out here. lat/lon and depth important, sufrace temp actually biggest r2 behind latlon. 

```{r biomass_gam}

load ("../Models/Biomass/Amblyraja_radiata_LB_full.Rdata")
summary (gam_LB)
```
...
for (x in full.terms){
  ...
  G <- gam (formula, family = gaussian, data = starry_ab_pred)
  
  # full model without that variable
 formula_drop <- as.formula (paste0 ("kg_log ~", (paste (full.terms[! full.terms %in% x], collapse = " + "))))
 G_drop <- gam (formula_drop, family = gaussian, data = filter (starry_ab_pred, !is.na (x)))
...
save (single.var.comp, file = "../Models/Biomass_summary_tables/starry_single_var_comp.RData")

```
salinity makes worse